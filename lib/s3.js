...
class S3 {
  constructor(config) {
    ...
    if (this.accessKeyId && this.secretAccessKey) {
      awsOptions.accessKeyId = this.accessKeyId
      awsOptions.secretAccessKey = this.secretAccessKey
    }
  ...
  createPromisifiedMethods(s3) {
    ...
    this.copyObject = promisify(s3.copyObject).bind(s3)
  }
  ...
  async copy(srckey, dstkey, options) {
    options = options || { }
    var srcbucket = options.srcbucket || this.bucket
    var params = {
      CopySource: `/${srcbucket}/${srckey}`,
      Bucket: options.dstbucket || this.bucket,
      Key: dstkey
    }
    var data = await this.copyObject(params)
    return data
  }
}
